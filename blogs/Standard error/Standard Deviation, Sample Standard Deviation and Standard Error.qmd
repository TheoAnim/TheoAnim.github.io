...
g1 <- ggplot() +
  ...
  xlim(2, max(n)) +
  labs(y = "SD/SSD/SE", title = "Normal Distribution")
```

```{r}
set.seed(1)
#simulate from gamma distribution with different sample sizes
samples2 <- map(n, \(n) rgamma(n, shape = 2, rate = 100))

#sample sd for each sample
SSD <- map_dbl(samples2, \(.x) sd(.x))

#SE of mean of each sample
SE <- map_dbl(samples2, \(.x) sd(.x)/sqrt(length(.x)))
